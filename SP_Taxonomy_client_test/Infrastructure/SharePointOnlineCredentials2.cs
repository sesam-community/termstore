...
{
    public sealed class SharePointOnlineCredentials2 : ICredentials
    {
        ...
        public SharePointOnlineCredentials2(string username, SecureString password)
        {
        ...
        internal string GetAuthenticationCookie(Uri url, bool refresh, bool alwaysThrowOnFailure)
        {
            ...
            Uri uri = new Uri(url, "/");
            string text = null;
            SharePointOnlineCredentials2.CookieCacheEntry cookieCacheEntry = (SharePointOnlineCredentials2.CookieCacheEntry)this.m_cachedCookies[uri];
            ...
            if (refresh)
            {
                ...
                if (!string.IsNullOrEmpty(text))
                {
                    ...
                    lock (this.m_lock)
                    {
                        this.m_cachedCookies[uri] = new SharePointOnlineCredentials2.CookieCacheEntry
                        {
                            Cookie = text,
